...
def _causal_conv1d_update_kernel_gpu(
    ...
    # for pid1 in range((dim - 1)//BLOCK_N + 1):
    # [BLOCK_N,] elements along the feature-dimension (channel)
    idx_feats = tl.program_id(1) * BLOCK_N + tl.arange(0, BLOCK_N)
    # idx_feats = pid1 * BLOCK_N + tl.arange(0, BLOCK_N)
    ...
    sub_idx_tokens = tl.arange(0, NP2_STATELEN - VAL)
    x_base = x_ptr + (idx_seq * stride_x_seq) + (idx_feats * stride_x_dim)  # [BLOCK_N]

    x_ptrs = (
        x_base[None, :] + (sub_idx_tokens * stride_x_token)[:, None]
    )  # [BLOCK_M, BLOCK_N]

    mask_x = (
        (sub_idx_tokens < seqlen)[:, None]
        & (idx_feats < dim)[None, :]
    )  # token-index  # token-index  # feature-index
    sub_loaded_x = tl.load(x_ptrs, mask_x, 0.0)
    full_x = tl.full([NP2_STATELEN, BLOCK_N], 0, x_ptr.dtype.element_ty)
    # [4，128] 0 <- [2,128]
    loaded_x = tl.insert_slice(full_x, sub_loaded_x, offsets=(VAL, 0), sizes=(NP2_STATELEN - VAL, BLOCK_N), strides=(1, 1))
    # [END] modified

    tl.debug_barrier()

    # new_conv_state = tl.where(mask, conv_state, loaded_x) # FIXME
    new_conv_state = loaded_x + conv_state 
    ...
    # [BEG] modified
    conv_state_ptrs_target = (
        conv_state_base[:,None] + (idx_tokens * stride_conv_state_tok)[None, :]
    )  # [BLOCK_N, BLOCK_M]
    mask = (idx_tokens < state_len)[None, :] & (idx_feats < dim)[:,None]
    tl.store(conv_state_ptrs_target, new_conv_state.trans(), mask)
    # [END] modified
